...
total_aircraft_costs  %>% mutate(price_total_fuel_CAD = total_fuel_consumed*(1.69/0.2199692)) %>%
  filter(Aircraft %in% c("C-150", "C-152", "C-172"))
```
...
total_aircraft_costs_year %>% mutate(price_total_fuel_CAD = total_fuel_consumed*(1.69/0.2199692)) %>%
  filter(Aircraft %in% c("C-150", "C-152", "C-172"))
```
...
my_cd2 %>% 
  ...
  group_by(Exercises_int) %>%
  summarize(Exercises_count = n()) %>%
  arrange(Exercises_int) %>%
  select(Exercises_int, Exercises_count) -> success_exercises
...
my_cd2 %>% 
  ...
  summarize(Exercises_count = n()) %>%
  arrange(Exercises_int) %>%
  select(Exercises_int, Exercises_count) -> unsuccessful_student_exercises
...
student_exercises %>%
  ggplot(aes(x=Exercises_int, y=Exercises_count)) + geom_bar(stat="identity") + facet_grid(.~ Student_type) + xlab("Exercises") + ylab("Number of Times Exercises Were Completed")
```
...
my_cd %>% 
  group_by(Student_ID) %>%
  summarize(N=n()) -> count_student_id

my_cd %>%
  left_join(count_student_id, by="Student_ID") 
```
